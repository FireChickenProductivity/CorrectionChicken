...
A correction option based on a correction rule is displayed if the percentage of the characters in the words matching the rule exceeds the `user.correction_chicken_correction_percentage_match_threshold` setting. This is 0.5 by default requiring more than half of the characters in the relevant words match. If a correction rule matches only a single word, then more than that percentage of the word must match for the corresponding correction option to be shown. 
